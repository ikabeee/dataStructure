...
string convertToLower(foodChoice)
{
    transform(foodChoice.begin(), foodChoice.end(), foodChoice.begin(), [](unsigned char c)
              { return lower(c); });
}
...
int main()
{
    ...
    do
    {
        ...
        if (decision == "s")
        {
            ...
            if (!flag)
            {
                cout << "No se encontró el alimento, intentando de nuevo...\n";
            }
        }
    ...
    for (int i = 0; i < foodSize; i++)
    {
        if (pieces[i] != 0)
        {
            cout << food[i] << " = $" << prices[i] << " * " << pieces[i] << "  = " << totalPerFood[i] << endl;
        }
    }
